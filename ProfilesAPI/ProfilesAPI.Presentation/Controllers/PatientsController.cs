...
public class PatientsController : ControllerBase
{
    ...
    [HttpPost]
    [ProducesResponseType(typeof(PatientInfoDTO), 201)]
    [ProducesResponseType(typeof(FailMessage), 400)]
    [ProducesResponseType(typeof(FailMessage), 403)]
    [ProducesResponseType(typeof(FailMessage), 404)]
    [ProducesResponseType(typeof(FailMessage), 408)]
    ...
    {
        var result = await _patientService.CreatePatientAsync(patientForCreateDTO);
        if (!result.IsComplited)
        {
            return new FailMessage(result.ErrorMessage, result.StatusCode);
        }

        return CreatedAtAction(nameof(GetPatientById), new { patientId = result.Value.Id }, result.Value);
    }
